...
# --- Classes para Lista Encadeada Simples (Histórico Global) ---
...
historico_global = LinkedList()  # Fila para histórico de ações globais (máximo 50 registros) (FIFO)
...
def registrar_acao_global(acao: str):
    """Registra uma ação no histórico global com timestamp."""
    timestamp = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    historico_global.append(f"[{timestamp}] {acao}")
...
def listar_historico_global():
    """Exibe o histórico de ações globais do sistema."""
    print("\n--- HISTÓRICO GLOBAL DE AÇÕES ---")
    registros = historico_global.list_all()
    if not registros:
